...
void MakeBatch(const std::vector<size_t>& tokens, size_t begin, size_t end,
    std::mt19937& rng, std::vector<size_t>& input_ids, std::vector<size_t>& target_ids) {
...
float EvaluateValidation(LanguageModel& model, const std::vector<size_t>& tokens,
    size_t validation_begin, size_t validation_end, std::mt19937& rng) {
    if (validation_end <= validation_begin + CONTEXT) {
        throw std::runtime_error("Validation set is too small");
    }

    std::uniform_int_distribution<size_t> dist(validation_begin,
    ...
    for (size_t batch = 0; batch < VALIDATION_BATCHES; batch++) {
        ...
        auto input = std::make_shared<Tensor>(std::vector<size_t>{
            BATCH_SIZE, CONTEXT}, input_data, TRAINING_DEVICE);

        auto targets = std::make_shared<Tensor>(std::vector<size_t>{
            BATCH_SIZE, CONTEXT}, target_data, TRAINING_DEVICE);
        ...
        if (error != cudaSuccess) {
            throw std::runtime_error(
                std::string("Validation loss cudaMemcpy failed: ") +
...
int main() {
    ...
    try {
        ...
        std::cout << "Dataset\n" << "  Characters: " << text.size() << "\n"
            << "  Tokens:     " << tokens.size() << "\n"
            << "  Train:      " << train_end << "\n"
            << "  Validation: " << validation_end - validation_begin << "\n\n";

        LanguageModel model(VOCAB_SIZE, EMBED_DIM, 
            BLOCKS, HEADS, HIDDEN, TRAINING_DEVICE);
        ...
        float initial_val_loss = EvaluateValidation(model,
                tokens, validation_begin, validation_end, validation_rng);

        float best_val_loss = initial_val_loss;
        size_t best_step = 0;

        std::cout << "Initial validation loss: " << initial_val_loss << "\n\n";
        std::cout << "Training...\n";
        ...
        for (size_t step = 1; step <= STEPS; step++) {
            ...
            auto input = std::make_shared<Tensor>(
                std::vector<size_t>{BATCH_SIZE, CONTEXT}, input_data, TRAINING_DEVICE);

            auto targets = std::make_shared<Tensor>(
                std::vector<size_t>{BATCH_SIZE, CONTEXT}, target_data, TRAINING_DEVICE);
            ...
            if (error != cudaSuccess) {
                throw std::runtime_error(
                    std::string("Training loss cudaMemcpy failed: ") +
                    cudaGetErrorString(error)
                );
            }
            ...
            model.UpdateAdamW(LEARNING_RATE, BETA1, BETA2, EPS, WEIGHT_DECAY);
            ...
            if (error != cudaSuccess) {
                throw std::runtime_error(
                    std::string("CUDA error after AdamW: ") +
                    cudaGetErrorString(error)
                );
            }
